...
#define STAT_OUTPUT_PATH     (1u << 4)  /* jitter= late= silence= wd_events= underruns= */
                       ...
                       uint64_t overruns, uint64_t underruns)
{
    ...
    if (flags & STAT_OUTPUT_PATH)
        n += snprintf(buf + n, sizeof(buf) - n,
                      " jitter=%.1fms late=%llu silence=%llu wd_events=%llu underruns=%llu",
                      jitter,
                      (unsigned long long)late,
                      (unsigned long long)silence,
                      (unsigned long long)wd_events,
                   ...
                   const watchdog_t      *wd)
{
    ...
    if (tel->prev_input_active && !input_active) {
        log_status(LOG_INFO, "input-end",
                   ...
                   jitter, late, silence, wd_events, overruns, underruns);
        log_now = true;
    }
